...
function compareArrays(arr1, arr2) {
  ...
  const result = arr1Json === arr2Json;
  ...
  if (typeof size === "number") {
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let j = 0; j < size; j++) {
        ...
        arrRowMatr.push(Math.floor(Math.random() * 10));
      }
      arrMatr.push(arrRowMatr);
    }
...
function traceBackward(n) {
  ...
  if (n === 1) console.trace("Reached the end");
}
...
function calculateTotalPrice(products) {
  ...
  let total = 0;
  products.forEach((elem) => (total += elem.price));
